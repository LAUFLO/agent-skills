...
AI 导演/分镜 Skill：把故事/小说扩展为完整制作蓝图——结构化剧本（series.json + 场景卡 + script.json）+ 图片/视频提示词 + 声音/VFX 规格 + 可执行制作计划。适用于"分镜"、"剧本"、"漫剧"、"短剧"、"视频提示词"、"把故事做成视频"等场景。

- 阶段 0~④ 人审管道，每阶段暂停确认；`series.json` 为跨会话真相源
- 含 21 个编剧 skill 蒸馏的素材库（风格样板/案例数据/喜剧规则/国产剧/学派替代路径/通用查表，按需查、主流程不载入）
