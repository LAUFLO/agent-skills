...
- 项目初始化：首次进入会话自动预建 `00-` 至 `05-` 骨架；只增不删，骨架与已有目录名冲突时停下给"认领 / 改名 / 忽略"三选项由用户裁决，空骨架目录不算阶段完成
- 分镜三件套（subject-registry / atmosphere-lock / shotlist）为唯一真相源；参考图资产绑定编号、版本只增不改
- 资产生成规格与 Agnes 集成（§7）：定妆照 16:9 三视图+头部特写；场景图"基准图 → 左/右/背面 → 俯视"五件套；图生视频参考图直接用在线 URL（禁 Data URI）；资产图质检一律派 subagent 读图（规避单会话 4 张图上限）
